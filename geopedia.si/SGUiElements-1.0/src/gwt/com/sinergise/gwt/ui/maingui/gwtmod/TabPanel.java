 ...
 * the user selects the various tabs associated with them. The tabs can contain arbitrary HTML.
 * <p>
 * This widget will <em>only</em> work in quirks mode. If your application is in Standards Mode, use
 * {@link TabLayoutPanel} instead.
 * </p>
 * <p>
 * <img class='gallery' src='doc-files/TabPanel.png'/>
 * </p>
 * <p>
 * Note that this widget is not a panel per se, but rather a {@link com.google.gwt.user.client.ui.Composite} that
 * aggregates a {@link com.google.gwt.user.client.ui.TabBar} and a {@link com.google.gwt.user.client.ui.DeckPanel}. It
 * does, however, implement {@link com.google.gwt.user.client.ui.HasWidgets}.
 * </p>
...
public class TabPanel extends Composite implements /* XXX TabListener, SourcesTabEvents, */ BeforeSelectionHandler<Integer>, SelectionHandler<Integer>, HasWidgets, HasAnimation, IndexedPanel.ForIsWidget, HasBeforeSelectionHandlers<Integer>, HasSelectionHandlers<Integer> {
	/**
	 * This extension of DeckPanel overrides the public mutator methods to prevent external callers from adding to the
	 * state of the DeckPanel.
	 * <p>
	 * Removal of Widgets is supported so that WidgetCollection.WidgetIterator operates as expected.
	 * </p>
	 * <p>
	 * We ensure that the DeckPanel cannot become of of sync with its associated TabBar by delegating all mutations to
	 * the TabBar to this implementation of DeckPanel.
	 * </p>
	 */
	 //XXX private -> protected
	protected static class TabbedDeckPanel extends DeckPanel {
		...
		protected void insertProtected(Widget w, String tabText, boolean asHTML, int beforeIndex) {
			...
			doInsert(w, beforeIndex);
		}

		private void doInsert(Widget w, int beforeIndex) {
			String wBefore = DOM.getStyleAttribute(w.getElement(), "width");
			String hBefore = DOM.getStyleAttribute(w.getElement(), "height");
			super.insert(w, beforeIndex);
	        DOM.setStyleAttribute(w.getElement(), "width", wBefore);
	        DOM.setStyleAttribute(w.getElement(), "height", hBefore);
	        
	        Element container = DOM.getParent(w.getElement());
	        DOM.setStyleAttribute(container, "width", "");
	        DOM.setStyleAttribute(container, "height", "");
	        DOM.setStyleAttribute(container, "padding", "");
	        DOM.setStyleAttribute(container, "margin", "");
	        
	        UIObject.setStyleName(container, "tabContainer "/*+w.getStyleName()*/);
		}
	...
	/**
	 * This extension of TabPanel overrides the public mutator methods to prevent external callers from modifying the
	 * state of the TabBar.
	 */
	// XXX private -> protected static
	protected static class UnmodifiableTabBar extends TabBar {
		private TabPanel owner; // XXX
		public UnmodifiableTabBar() {
			super();
		}
		
		public UnmodifiableTabBar(ComplexPanel container) {
			super(container);
		}
		...
		protected SimplePanel createTabTextWrapper() {
			return owner.createTabTextWrapper();
		}
		
		protected void initOwner(TabPanel owner) {
			this.owner = owner;
		}
	}
	 ...
	 */
	public TabPanel() {
		this(new VerticalPanel(), new UnmodifiableTabBar());
	}

	public TabPanel(CellPanel contentPanel, UnmodifiableTabBar tabBar) {
		super();
		tabBar.initOwner(this);
		this.tabBar = tabBar;
		this.deck = new TabbedDeckPanel(tabBar);
		contentPanel.add(tabBar);
		contentPanel.add(deck);

		contentPanel.setCellHeight(deck, "100%");
		tabBar.setWidth("100%");
	...
	/**
	 * Adds a widget to the tab panel. If the Widget is already attached to the TabPanel, it will be moved to the
	 * right-most index.
	 * 
	...
	/**
	 * Gets the deck panel within this tab panel. Adding or removing Widgets from the DeckPanel is not supported and
	...
	/**
	 * Gets the tab bar within this tab panel. Adding or removing tabs from from the TabBar is not supported and will
	...
	/**
	 * Inserts a widget into the tab panel. If the Widget is already attached to the TabPanel, it will be moved to the
	 * requested index.
	 * 
...
// XXX
	public void onBeforeSelection(BeforeSelectionEvent<Integer> event) {
		BeforeSelectionEvent<Integer> e = BeforeSelectionEvent.fire(this, event.getItem());
...
// XXX
	public void onSelection(SelectionEvent<Integer> event) {
		...
		SelectionEvent.fire(this, event.getSelectedItem());
	}
