...
/**
 * A horizontal bar of folder-style tabs, most commonly used as part of a {@link com.google.gwt.user.client.ui.TabPanel}
 * .
 * <p>
...
public class TabBar extends Composite implements /* XXX SourcesTabEvents,*/ HasBeforeSelectionHandlers<Integer>, HasSelectionHandlers<Integer>, ClickListener, KeyboardListener {

	/**
	 * Set of characteristic interfaces supported by {@link TabBar} tabs. Note that this set might expand over time, so
	 * implement this interface at your own risk.
	 */
	...
	/**
	 * <code>ClickDelegatePanel</code> decorates any widget with the minimal amount of machinery to receive clicks for
	 * delegation to the parent. {@link SourcesClickEvents} is not implemented due to the fact that only a single
	 * observer is needed.
	 */
	protected class ClickDelegatePanel extends Composite implements Tab {
		private SimplePanel	focusablePanel;
		private boolean		enabled	= true;

		ClickDelegatePanel(Widget child) {
			//XXX
			focusablePanel = new SimplePanel(FocusImpl.getFocusImplForPanel().createFocusable()) {};
		...
		public boolean getWordWrap() {
			if (hasWordWrap()) { return ((HasWordWrap)focusablePanel.getWidget()).getWordWrap(); }
	...
	//XXX
	protected final ComplexPanel	panel;
	...
	//XXX
	public TabBar(ComplexPanel containerPanel) {
		...
		//XXX
		if (panel instanceof HasVerticalAlignment) {
			((HasVerticalAlignment)panel).setVerticalAlignment(HasVerticalAlignment.ALIGN_BOTTOM);
		}
		...
		panel.add(first);
		panel.add(rest);
		//XXX Added by mkadunc
		styleFirstRest(panel, first, rest);
		//End added
	...
	// XXX Added by mkadunc
	protected void styleFirstRest(ComplexPanel panel, HTML first, HTML rest) {
		first.setHeight("100%");
		rest.setHeight("100%");
		if (panel instanceof CellPanel) {
			((CellPanel)panel).setCellHeight(first, "100%");
			((CellPanel)panel).setCellWidth(rest, "100%");
		}
	}
	...
	/**
	 * Gets the given tab. This method is final because the Tab interface will expand. Therefore it is highly likely
	...
	public final Tab getTab(int index) {
		if (index >= getTabCount()) { return null; }
	...
	/**
	 * @deprecated add a {@link BeforeSelectionHandler} instead. Alternatively, if you need to access to the individual
	 *             tabs, add a click handler to each {@link Tab} element instead.
	 */
	@Deprecated
	public void onClick(Widget sender) {}

	/**
	 * @deprecated add a key down handler to the individual {@link Tab} objects instead.
	 */
	@Deprecated
	public void onKeyDown(Widget sender, char keyCode, int modifiers) {}

	/**
	 * @deprecated this method has been doing nothing for the entire last release, if what you wanted to do was to
	 *             listen to key press events on tabs, add the key press handler to the individual tab wrappers instead.
	 */
	@Deprecated
	...
	/**
	 * @deprecated this method has been doing nothing for the entire last release, if what you wanted to do was to
	 *             listen to key up events on tabs, add the key up handler to the individual tab wrappers instead.
	 */
	@Deprecated
	...
	/**
	 * Sets a tab's contents via HTML. Use care when setting an object's HTML; it is an easy way to expose script-based
	 * security problems. Consider using {@link #setTabText(int, String)} or {@link #setTabHTML(int, SafeHtml)} whenever
	 * possible.
	 * 
	...
	/**
	 * Create a {@link SimplePanel} that will wrap the contents in a tab. Subclasses can use this method to wrap tabs in
	 * decorator panels.
	 * 
...
//XXX
		((InsertPanel)panel).insert(delWidget, beforeIndex + 1);
	...
	private void checkInsertBeforeTabIndex(int beforeIndex) {
		if ((beforeIndex < 0) || (beforeIndex > getTabCount())) { throw new IndexOutOfBoundsException(); }
	}

	private void checkTabIndex(int index) {
		if ((index < -1) || (index >= getTabCount())) { throw new IndexOutOfBoundsException(); }
	}

	/**
	 * Selects the tab corresponding to the widget for the tab. To be clear the widget for the tab is not the widget
	 * INSIDE of the tab; it is the widget used to represent the tab itself.
	 * 
	...
	private boolean selectTabByTabWidget(Widget tabWidget) {
		...
		for (int i = 1; i < numTabs; ++i) {
			if (panel.getWidget(i) == tabWidget) { return selectTab(i - 1); }
		}
