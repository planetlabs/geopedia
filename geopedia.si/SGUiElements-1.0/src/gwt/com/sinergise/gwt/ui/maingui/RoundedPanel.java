 ...
 * @version 1.3
 * 
 * Use GWT's decorated panel instead
 */
public class RoundedPanel extends SimplePanel {

  /**
   * <code>NONE</code> NO rounded corner
   */
   ...
   */
  private final static String RPSTYLE = "cbg-RPP";
  ...
  public RoundedPanel(int corners, int cornerHeight) {
    super(DOM.createDiv());
    body = getElement();
    //CSS.background(body, CSS.TRANSPARENT);
    ...
    if (inMask(corners, TOP)) {
      ...
      for (int i = 0; i < cornerHeight; ++i) {
        divt[i] = addLine(ct, cornerHeight - (i + 1), i);
      }
    }
    ...
    if (inMask(corners, BOTTOM)) {
      ...
      for (int i = cornerHeight - 1; i >= 0; --i) {
        divb[i] = addLine(cb, cornerHeight - (i + 1), i);
      }
    }
  ...
  public void setCornerStyleName(String style) {
    if (null != divt[0]) {
      for (int i = 0; i < cornerHeight; ++i) {
    	  CSS.className(divt[i], style);
      }
    }
    if (null != divb[0]) {
      for (int i = 0; i < cornerHeight; ++i) {
    	  CSS.className(divb[i], style);
      }
    }
  }
...
protected Element getContainerElement() {
   ...
   */
  private Element addLine(int corner, int heightIndex, int nline) {
	...
	if (nline == 0) {
	    ...
	    if (corner == TOP) {
		    DOM.setStyleAttribute(div, "borderWidth",
			        "0px 0px " + RoundedPanel.CORNERBORDER[cornerHeight - 1][heightIndex]);
		}
		else
		{
		    DOM.setStyleAttribute(div, "borderWidth",
    		        RoundedPanel.CORNERBORDER[cornerHeight - 1][heightIndex] + " 0px 0px");
		}
	}
	else
	{
