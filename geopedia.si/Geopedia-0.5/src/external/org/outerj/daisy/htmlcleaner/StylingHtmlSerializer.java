 ...
 * 
 * <p>
 * This serializer is not meant as a general purpose serializer. Rather it is
 * used as part of a HTML cleaning pipeline whose goal is to normalize html
 * before storing it in a CMS.
 * <p>
 * 
 * <p>
 * This serializer isn't designed or tested for performance, but that doesn't
 * matter since it's only used for update operations and for relatively small
 * content blurbs.
 * </p>
 * 
 * <p>
 * The serializer will limit the output width to a certain number of characters.
 * It can be configured to output a variable number of characters around the
 * start and end tags of certain elements. Sequences of multiple whitespace
 * characters are collapsed.
 * </p>
 * 
 * <p>
 * The input must contain the html root tag.
 * </p>
 * 
 * <p>
 * The output encoding is always UTF-8. Note that this can't simply be changed
 ...
 * checks are not required). The serializer does not check for characters that
 * are illegal in XML.
 * </p>
 */
class StylingHtmlSerializer implements ContentHandler
{
	...
	{
		// Note: this serializer assumes hardcoded UTF-8. Otherwise it would
		// need extra
	...
	private String escapeReservedCharacters(String text)
	{
		...
		for (int i = 0; i < text.length(); i++) {
			...
			case '&':
				...
				break;
			default:
				// if the character is not in the range allowed by XML, simply
				// skip it
				// (Note: neko html doesn't removed these characters when
				// parsing, so we do it here)
	...
	/**
	 * Class that keeps track of the current line being written, and whether a
	 * new line should be started and such. All output should happen via this
	 * class.
	 */
				...
				if (inPreElement) {
					...
					if (words.size() > 0) {
						...
						while (wordsIt.hasNext()) {
							...
							if (line.getLength() > 0
							                && line.getLength() + word.length() + 1 > template.maxLineWidth) {
	...
	/**
	 * The Line class keeps track of which items are on the current line, and
	 * helps finding where the line can be split if it becomes to long. Breaks
	 * are only allowed at whitespace locations or at 'block' tags, NOT where
	 * two inline tags or an inline tag and a word are next to each other
	 * whithout whitespace between them.
	 */
	private class Line
	{
		 ...
		 * 
		 * @param until
		 *            index of last item to be included
		 */
		...
		/**
		 * If a word it to be added, but it doesn't fit anymore on the line,
		 * this method is used to try to empty the line, either completely or as
		 * far as possible, depending on whether there is an apropriate place to
		 * split the line.
		 * 
		...
		/**
		 * This method should only be called if the last item on the line is a
		 * Word or a inline Tag.
		 * 
		 * Returns -1 if there's not suitable split point, otherwise returns the
		 * index of the last item before the possible split point.
		 */
		private int searchSplitPoint()
		{
			if (lineItems.size() < 2) {
				// there is only one item on the line, and it cannot be
				// disconnected from the next item
				return -1;
			}
