...
class GeckoCorruptTagCleaner
{
	 ...
	 * 
	 * <p>
	 * The Gecko Midas editor (or Mozilla Composer for that matter) sometimes
	 * leaves things like '&lt;&gt;' or '&lt; /&gt;' (for those reading the
	 * source: it leaves the unescaped tags), thus tags without names. This is
	 * easily reproduceable by hitting enter twice followed by backspace twice
		...
		while (i < inputChars.length) {
			...
			if (c == '<' && i + 3 < inputChars.length) {
				if (inputChars[i + 1] == '>') {
					i = i + 2;
					continue;
				} else if (/* c == '<' && */inputChars[i + 1] == ' ' && inputChars[i + 2] == '/'
				                && inputChars[i + 3] == '>') {
					i = i + 4;
					continue;
