...
/**
 * Builds a {@link HtmlCleanerTemplate} based on a XML configuration. The
 * configuration describes such things as which elements and attributes should
 * be kept, or how wide the output should be. See the example config files.
 * 
 * <p>
 * Instances of this class are not thread safe and not reusable, in other words
 * construct a new HtmlCleanerFactory each time you need it.
 */
public class HtmlCleanerFactory
{
	...
	private static boolean checkNameSpace(Node n){
		return n.getNamespaceURI() == null ||  n.getNamespaceURI().length() == 0;
	}
	...
	public HtmlCleanerTemplate buildTemplate(InputSource is) throws Exception
	{
		...
		if (!(docEl.getLocalName().equals("htmlcleaner") && (docEl.getNamespaceURI() == null ||  docEl.getNamespaceURI().length() == 0))) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node instanceof Element) {
				if (checkNameSpace(node) && node.getLocalName().equals("cleanup")) {
					handleCleanupNode((Element) node);
				} else if (checkNameSpace(node) && node.getLocalName().equals("serialization")) {
					handleSerializationNode((Element) node);
				} else {
					throw new Exception("Error in htmlcleaner config: unexpected element: "
					                + node.getNodeName());
				}
			}
		}
	...
	private void handleCleanupNode(Element cleanupEl) throws Exception
	{
		...
		for (int k = 0; k < cleanupNodes.getLength(); k++) {
			...
			if (node instanceof Element) {
				if (checkNameSpace(node) && node.getLocalName().equals("allowed-span-classes")) {
					String[] classes = getClassChildren((Element) node);
					for (String clazz : classes)
						template.addAllowedSpanClass(clazz);
				} else if (checkNameSpace(node)
				                && node.getLocalName().equals("allowed-div-classes")) {
					String[] classes = getClassChildren((Element) node);
					for (String clazz : classes)
						template.addAllowedDivClass(clazz);
				} else if (checkNameSpace(node)
				                && node.getLocalName().equals("allowed-para-classes")) {
					String[] classes = getClassChildren((Element) node);
					for (String clazz : classes)
						template.addAllowedParaClass(clazz);
				} else if (checkNameSpace(node)
				                && node.getLocalName().equals("allowed-pre-classes")) {
					String[] classes = getClassChildren((Element) node);
					for (String clazz : classes)
						template.addAllowedPreClass(clazz);
				} else if (checkNameSpace(node) && node.getLocalName().equals("drop-div-classes")) {
					String[] classes = getClassChildren((Element) node);
					for (String clazz : classes)
						template.addDropDivClass(clazz);
				} else if (checkNameSpace(node) && node.getLocalName().equals("allowed-elements")) {
					handleAllowedElementsNode((Element) node);
				} else if (checkNameSpace(node)
				                && node.getLocalName().equals("img-alternate-src-attr")) {
					String name = ((Element) node).getAttribute("name");
					if (name.equals(""))
						throw new Exception(
						                "Error in htmlcleaner config: missing name attribute on img-alternate-src-attr");
					template.setImgAlternateSrcAttr(name);
				} else if (checkNameSpace(node)
				                && node.getLocalName().equals("link-alternate-href-attr")) {
					String name = ((Element) node).getAttribute("name");
					if (name.equals(""))
						throw new Exception(
						                "Error in htmlcleaner config: missing name attribute on link-alternate-href-attr");
					template.setLinkAlternateHrefAttr(name);
				} else {
					throw new Exception("Error in htmlcleaner config: unexpected element "
					                + node.getNodeName() + " inside " + cleanupEl.getNodeName());
				}
			}
		}
	...
	private String[] getClassChildren(Element element) throws Exception
	{
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			Node node = nodeList.item(i);
			if (node instanceof Element) {
				if (checkNameSpace(node) && node.getLocalName().equals("class")) {
					Node text = node.getFirstChild();
					if (text instanceof Text) {
						classes.add(((Text) text).getData());
					} else {
						throw new Exception(
						                "Error in htmlcleaner: element class does not have a text node child");
					}
				} else {
					throw new Exception("Error in htmlcleaner config: unexpected element: "
					                + node.getNodeName() + " as child of " + element.getNodeName());
				}
			}
		}
	...
	private void handleAllowedElementsNode(Element element) throws Exception
	{
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (node instanceof Element) {
				if (checkNameSpace(node) && node.getLocalName().equals("element")) {
					String name = ((Element) node).getAttribute("name");
					if (name.equals(""))
						throw new Exception(
						                "Error in htmlcleaner config: missing name attribute on 'element' element");
	...
	private String[] getAttributeChildren(Element element) throws Exception
	{
		...
		for (int i = 0; i < children.getLength(); i++) {
			Node node = children.item(i);
			if (node instanceof Element) {
				if (checkNameSpace(node) && node.getLocalName().equals("attribute")) {
					String name = ((Element) node).getAttribute("name");
					if (name.equals(""))
						throw new Exception(
						                "Error in htmlcleaner config: missing name attribute on attribute element");
					names.add(name);
				} else {
					throw new Exception("Error in htmlcleaner config: unexpected element: '"
					                + node.getNodeName() + "' as child of " + element.getNodeName());
	...
	private void handleSerializationNode(Element element) throws Exception
	{
		...
		for (int i = 0; i < children.getLength(); i++) {
			Node node = children.item(i);
			if (node instanceof Element) {
				if (checkNameSpace(node) && node.getLocalName().equals("linewidth")) {
					String value = ((Element) node).getAttribute("value");
					if (value.equals(""))
						throw new Exception(
						                "Error in htmlcleaner config: missing value attribute on linewidth element.");
					int intValue = Integer.parseInt(value);
					template.setMaxLineWidth(intValue);
				} else if (checkNameSpace(node) && node.getLocalName().equals("elements")) {
    ...
    private static HtmlCleanerTemplate defaultTemplate;
    
	public synchronized static HtmlCleanerTemplate defaultTemplate() throws Exception
    {
		if (defaultTemplate == null) {
			HtmlCleanerFactory factory = new HtmlCleanerFactory();
			InputSource is = new InputSource(HtmlCleanerFactory.class.getClassLoader().getResourceAsStream("org/outerj/daisy/htmlcleaner/cleanerconf.xml"));
			defaultTemplate = factory.buildTemplate(is);
		}
